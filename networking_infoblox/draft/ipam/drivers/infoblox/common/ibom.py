...
class IPBackend(object):
    ...
    def get_network(self, network_view, cidr):
        ...
        fields = ['options', 'members']
        network = self.ibom._get_infoblox_object_or_none(obj_type, payload,
                                                         return_fields=fields)
        if not network:
            raise ib_ex.InfobloxNetworkNotAvailable(network_view=network_view,
                                                    cidr=cidr)
        return ibo.Network.from_dict(network)
...
class InfobloxObjectManager(object):
    ...
    def create_network_view(self, network_view, extattrs,
        ...
        payload = {'name': network_view, 'extattrs': extattrs}

        # no need to add delegate parameter
        #self._add_delegate_member(payload, delegate_member)

        return self._create_infoblox_object('networkview', payload)

    def delete_network_view(self, network_view):
        # never delete default network view
        if network_view == 'default':
            return
        payload = {'name': network_view}
        self._delete_infoblox_object('networkview', payload)

    def create_dns_view(self, network_view, dns_view):
        payload = {'name': dns_view, 'network_view': network_view}
        return self._create_infoblox_object('view', payload)

    def delete_dns_view(self, network_view):
        payload = {'name': network_view}
        self._delete_infoblox_object('view', payload)

    def get_network(self, network_view, cidr):
        ip_backend = IPBackendFactory.get(self, cidr)
        return ip_backend.get_network(network_view, cidr)
    ...
    def _create_infoblox_object(self, obj_type, payload,
        ...
        if check_if_exists or update_if_exists:
            ib_obj = self._get_infoblox_object_or_none(obj_type, payload)
            if ib_obj:
    ...
    def _get_infoblox_object_or_none(self, obj_type, payload,
        ...
        # Ignore 'extattrs' for get_object, since this field is not searchable
        search_payload = {}
        for key in payload:
            if key is not 'extattrs':
                search_payload[key] = payload[key]
        ib_obj = self.connector.get_object(obj_type, search_payload,
