...
class InfobloxNetworkView(InfobloxObject):
    def __init__(self, name, extattrs=None):
        ...
        self.payload = {'name': name, 'extattrs': extattrs}
        if extattrs:
            self.create_data = {'extattrs': extattrs}
...
class InfobloxDNSView(InfobloxObject):
    def __init__(self, dnsview_name, netview_name=None):
        ...
        self.payload = {'name': dnsview_name}
        if netview_name:
            self.payload = {'network_view': netview_name}
...
class InfobloxNetwork(InfobloxObject):
    def __new__(cls, net_view_name, cidr, nameservers=None,
        ...
        if InfobloxObject._determine_ip_version(cidr) == 6:
            return super(InfobloxNetwork, cls).__new__(InfobloxNetworkV6,
                                    net_view_name, cidr, nameservers,
                                    members, gateway_ip, dhcp_trel_ip,
                                    network_extattrs)
        else:
            return super(InfobloxNetwork, cls).__new__(InfobloxNetworkV4,
                                    net_view_name, cidr, nameservers,
                                    members, gateway_ip, dhcp_trel_ip,
                                    network_extattrs)
    ...
    def __init__(self, obj_type, ip_ver, net_view_name, cidr, nameservers=None,
        ...
        self.payload = {'network_view': net_view_name,
                        'network': cidr}
        self.return_fields = ['options', 'members']

        if network_extattrs:
            self.create_data = {'extattrs': network_extattrs}

        members_struct = []
        if members:
            members_struct = [member.specifier for member in members]

        self.create_data = {'members': members_struct}
        ...
        if dhcp_options:
            self.create_data = {'options': dhcp_options}
...
class InfobloxNetworkV4(InfobloxNetwork):
        def __init__(self, net_view_name, cidr, nameservers=None,
            ...
            super(InfobloxNetworkV4, self).__init__('network', 4,
                                    net_view_name, cidr, nameservers,
                                    members, gateway_ip, dhcp_trel_ip,
                                    network_extattrs)
...
class InfobloxNetworkV6(InfobloxNetwork):
        def __init__(self, net_view_name, cidr, nameservers=None,
            ...
            super(InfobloxNetworkV6, self).__init__('ipv6network', 6,
                                    net_view_name, cidr, nameservers,
                                    members, gateway_ip, dhcp_trel_ip,
                                    network_extattrs)
...
class InfobloxHostRecord(InfobloxObject):
    def __new__(cls, dnsview_name, ip, name=None, extattrs=None):
        if InfobloxObject._determine_ip_version(ip) == 6:
            return super(InfobloxHostRecord, cls).__new__(InfobloxHostRecordV6,
                                            dnsview_name, ip, name, extattrs)
        else:
            return super(InfobloxHostRecord, cls).__new__(InfobloxHostRecordV4,
                                            dnsview_name, ip, name, extattrs)

    def __init__(self, obj_type, ip_ver, dnsview_name, ip, name, extattrs):
        super(InfobloxHostRecord, self).__init__(obj_type, ip_ver)

        self.payload = {'view': dnsview_name}
...
class InfobloxHostRecordV4(InfobloxHostRecord):
    def __init__(self, dnsview_name, ip, name=None, extattrs=None):
        super(InfobloxHostRecordV4, self).__init__('record:host', 4,
                                    dnsview_name, ip, name, extattrs)
        self.payload = {'ipv4addr': ip}
        self.return_fields = ['ipv4addrs']

class InfobloxHostRecordV6(InfobloxHostRecord):
    def __init__(self, dnsview_name, ip, name=None, extattrs=None):
        super(InfobloxHostRecordV6, self).__init__('record:host', 6,
                                    dnsview_name, ip, name, extattrs)
        self.payload = {'ipv6addr': ip}
...
class InfobloxIPRange(InfobloxObject):
    def __new__(cls, netview_name, start_addr, end_addr,
        ...
        if InfobloxObject._determine_ip_version(start_addr) == 6:
            return super(InfobloxIPRange, cls).__new__(InfobloxIPRangeV6,
                                            netview_name, start_addr, end_addr,
                                            cidr, disable, extattrs)
        else:
            return super(InfobloxIPRange, cls).__new__(InfobloxIPRangeV4,
                                            netview_name, start_addr, end_addr,
                                            cidr, disable, extattrs)
    ...
    def __init__(self, obj_type, ip_ver, netview_name, start_addr, end_addr,
        ...
        self.payload = {'start_addr': start_addr,
                        'end_addr': end_addr,
                        'network_view': netview_name}
        if cidr:
            self.payload = {'network': cidr}

        if extattrs:
            self.create_data = {'extattrs': extattrs}
        if disable:
            self.create_data = {'disable': disable}

class InfobloxIPRangeV4(InfobloxIPRange):
    def __init__(self, netview_name, start_addr, end_addr,
                 cidr=None, disable=None, extattrs=None):
        super(InfobloxIPRangeV4, self).__init__('range', 4,
                                    netview_name, start_addr, end_addr,
                                    cidr, disable, extattrs)
...
class InfobloxIPRangeV6(InfobloxIPRange):
    def __init__(self, netview_name, start_addr, end_addr,
                 cidr=None, disable=None, extattrs=None):
        super(InfobloxIPRangeV6, self).__init__('ipv6range', 6,
                                        netview_name, start_addr, end_addr,
                                        cidr, disable, extattrs)
...
class InfobloxObjectManager(object):
    ...
    def create_network(self, net_view_name, cidr, nameservers=None,
        ...
        network_obj = InfobloxNetwork(
                        net_view_name, cidr, nameservers,
                        members, gateway_ip, dhcp_trel_ip,
                        network_extattrs)
        return self._create_infoblox_object(
                                    network_obj, check_if_exists=False)

    def get_network(self, network_view, cidr):
        network_obj = InfobloxNetwork(net_view_name, cidr)
    ...
    def _create_infoblox_object(self, create_obj,
        ...
        addtional_create_kwargs = create_obj.create_data
    ...
    def _get_infoblox_object_or_none(self, get_obj, proxy=False):
        ...
        # is only used for creating object is only added to create_data attribute
        #for key in payload:
