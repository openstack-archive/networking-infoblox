...
class IPv4Backend(IPBackend):
    ...
    def add_ip_to_record(self, host_record, ip, mac, use_dhcp):
        host_record.ips.append(objects.IPv4(ip, mac, use_dhcp))
    ...
    def find_hostname(self, dns_view, hostname):
        ...
        if hr_dict:
            hr_obj = objects.HostRecordIPv4.from_dict(hr_dict)
            return hr_obj
        return None
...
class IPv6Backend(IPBackend):
    ...
    def add_ip_to_record(self, host_record, ip, mac, use_dhcp=True):
        host_record.ips.append(objects.IPv6(ip, mac, use_dhcp))
    ...
    def find_hostname(self, dns_view, hostname):
        ...
        hr_dict = self.ibom._get_infoblox_object_or_none('record:host',
                                                         ...
                                                         return_fields=fields)
        if hr_dict:
            hr_obj = objects.HostRecordIPv6.from_dict(hr_dict)
            return hr_obj
        return None
...
class InfobloxObject(object):
    ...
    def _ip_to_str(self, ip):
        if isinstance(ip, InfobloxIPAllocationObject):
            return ip.next_available_ip
        return ip

    def _prepare_ip(self, ip, mac, use_dhcp):
        ip_str = self._ip_to_str(ip)
        ip_type = self.ip_type()

        return [ip_type(ip_str, mac, use_dhcp).to_dict()]


class InfobloxIPAllocationObject(object):

    def __init__(self, address, next_available_ip):
        self.ip_version = InfobloxObject._determine_ip_version(address)
...
class InfobloxNetworkView(InfobloxObject):
    def __init__(self, name, extattrs=None):
        ...
        if extattrs:
            self.payload.update({'extattrs': extattrs})
...
class InfobloxNetwork(InfobloxObject):
    @classmethod
    def load(cls, net_view_name, cidr, nameservers=None,
             members=None, gateway_ip=None, dhcp_trel_ip=None,
             network_extattrs=None):
        if InfobloxObject._determine_ip_version(cidr) == 6:
            return InfobloxNetworkV6(net_view_name, cidr, nameservers,
                                     members, gateway_ip, dhcp_trel_ip,
                                     network_extattrs)
        else:
            return InfobloxNetworkV4(net_view_name, cidr, nameservers,
                                     members, gateway_ip, dhcp_trel_ip,
                                     network_extattrs)
...
class InfobloxHostRecord(InfobloxObject):
    @classmethod
    def load(cls, dnsview_name, ip, name=None, mac=None, extattrs=None,
             use_dhcp=True):
        if InfobloxObject._determine_ip_version(ip) == 6:
            return InfobloxHostRecordV6(dnsview_name, ip, name, mac, extattrs,
                                        use_dhcp)
        else:
            return InfobloxHostRecordV4(dnsview_name, ip, name, mac, extattrs,
                                        use_dhcp)

    def __init__(self, obj_type, ip_ver, dnsview_name, name, extattrs):
...
class InfobloxHostRecordV4(InfobloxHostRecord):
    def __init__(self, dnsview_name, ip, name, mac, extattrs,
                 use_dhcp):
        super(InfobloxHostRecordV4, self).__init__(
            'record:host', 4, dnsview_name, name, extattrs)
        ips = self._prepare_ip(ip, mac, use_dhcp)
        self.payload.update({'ipv4addrs': ips})
        self.return_fields = ['ipv4addrs']

    def ip_type(self):
        return objects.IPv4

    def parsing_class(self):
        return objects.HostRecordIPv4

    def add_ip(self, ip, mac, use_dhcp):
        new_ip = objects.IPv4(ip, mac, use_dhcp)
        self.payload['ipv4addrs'].append(new_ip)

    def delete_ip(self, ip):
        self.payload['ipv4addrs'].remove(ip)
...
class InfobloxHostRecordV6(InfobloxHostRecord):
    def __init__(self, dnsview_name, ip, name, mac, extattrs,
        ...
        super(InfobloxHostRecordV6, self).__init__(
            'record:host', 6, dnsview_name, name, extattrs)
        ips = self._prepare_ip(ip, mac, use_dhcp)
        self.payload.update({'ipv6addrs': ips})
...
class InfobloxIPRange(InfobloxObject):
    @classmethod
    def load(cls, netview_name, start_addr, end_addr,
             cidr=None, disable=None, extattrs=None):
        if InfobloxObject._determine_ip_version(start_addr) == 6:
            return InfobloxIPRangeV6(netview_name, start_addr, end_addr,
                                     cidr, disable, extattrs)
        else:
            return InfobloxIPRangeV4(netview_name, start_addr, end_addr,
                                     cidr, disable, extattrs)
...
class InfobloxFixedAddress(InfobloxObject):
    @classmethod
    def load(cls, network_view, ip, mac=None, extattrs=None):
        if InfobloxObject._determine_ip_version(ip) == 6:
            return InfobloxFixedAddressV6(network_view, ip, mac,
                                          extattrs)
        else:
            return InfobloxFixedAddressV4(network_view, ip, mac,
                                          extattrs)

    def __init__(self, obj_type, ip_ver, network_view, extattrs):
        super(InfobloxFixedAddress, self).__init__(obj_type, ip_ver)

        self.payload.update({'network_view': network_view})

        if extattrs:
            self.create_data.update({'extattrs': extattrs})


class InfobloxFixedAddressV4(InfobloxFixedAddress):
    def __init__(self, network_view, ip, mac, extattrs):
        super(InfobloxFixedAddressV4, self).__init__(
            'fixedaddress', 4, network_view, extattrs)
        self.payload.update({'ipv4addr': self._ip_to_str(ip)})
        if mac:
            self.payload.update({'mac': mac})

    def parsing_class(self):
        return objects.FixedAddressIPv4


class InfobloxFixedAddressV6(InfobloxFixedAddress):
    def __init__(self, network_view, ip, mac, extattrs):
        super(InfobloxFixedAddressV6, self).__init__(
            'ipv6fixedaddress', 6, network_view, extattrs)
        self.payload.update({'ipv6addrs': self._ip_to_str(ip)})
        if mac:
            self.payload.update({'duid': ib_utils.generate_duid(mac)})

    def parsing_class(self):
        return objects.FixedAddressIPv6


class InfobloxDNSRecord(InfobloxObject):
    @classmethod
    def load(cls, dns_view, ip, name, bind_list, extattrs=None, mode='create'):
        records = []
        if InfobloxObject._determine_ip_version(ip) == 6:
            if 'record:aaaa' in bind_list:
                records.append(InfobloxAaaaRecord(ip, dns_view,
                                                  name, extattrs, mode))
            if 'record:ptr' in bind_list:
                records.append(InfobloxPtrRecordV6(ip, dns_view,
                                                   name, extattrs, mode))
        else:
            if 'record:a' in bind_list:
                records.append(InfobloxARecord(ip, dns_view,
                                               name, extattrs, mode))
            if 'record:ptr' in bind_list:
                records.append(InfobloxPtrRecordV4(ip, dns_view,
                                                   name, extattrs, mode))
        return records

    def __init__(self, obj_type, ip_ver, dns_view, extattrs):
        super(InfobloxDNSRecord, self).__init__(obj_type, ip_ver)

        self.payload.update({'view': dns_view})

        if extattrs:
            self.create_data.update({'extattrs': extattrs})


class InfobloxARecord(InfobloxDNSRecord):
    def __init__(self, ip, dns_view, name, extattrs, mode):
        super(InfobloxARecord, self).__init__(
            'record:a', 4, dns_view, extattrs)
...
class InfobloxAaaaRecord(InfobloxDNSRecord):
    def __init__(self, ip, dns_view, name, extattrs, mode):
        super(InfobloxAaaaRecord, self).__init__(
            'record:aaaa', 6, dns_view, extattrs)
...
class InfobloxPtrRecordV4(InfobloxDNSRecord):
    def __init__(self, ip, dns_view, name, extattrs, mode):
        super(InfobloxPtrRecordV4, self).__init__(
            'record:ptr', 4, dns_view, extattrs)
        self.payload.update({'ipv4addr': ip})
        if mode == 'create':
            self.create_data.update({'ptrdname': name})
        else:
            self.payload.update({'ptrdname': name})


class InfobloxPtrRecordV6(InfobloxDNSRecord):
    def __init__(self, ip, dns_view, name, extattrs, mode):
        super(InfobloxPtrRecordV6, self).__init__(
            'record:ptr', 6, dns_view, extattrs)
        if mode == 'create':
            self.payload.update({'ipv6addr': ip})
            self.create_data.update({'ptrdname': name})
        else:
...
class InfobloxObjectManager(object):
    ...
    def create_network_view(self, network_view, extattrs):
        nv_obj = InfobloxNetworkView.load(network_view, extattrs)
    ...
    def create_network(self, net_view_name, cidr, nameservers=None,
        ...
        network_obj = InfobloxNetwork.load(
            net_view_name, cidr, nameservers, members,
    ...
    def get_network(self, network_view, cidr):
        network_obj = InfobloxNetwork.load(network_view, cidr)
        net = self._get_infoblox_object_or_none(network_obj)
        if not net:
    ...
    def create_ip_range(self, network_view, start_ip, end_ip, network,
        ...
        range_obj = InfobloxIPRange.load(network_view, start_ip, end_ip,
                                         network, extattrs=range_extattrs)
        range_obj.create_data.update({'disable': disable})
    ...
    def has_networks(self, network_view_name):
        networks = InfobloxNetwork.load(network_view_name, None)
        try:
            ib_network = self._get_infoblox_object_or_none(networks)
            return bool(ib_network)
        except ib_ex.InfobloxSearchError:
            return False

    def network_exists(self, network_view, cidr):
        network = InfobloxNetwork.load(network_view, cidr)
        try:
            ib_network = self._get_infoblox_object_or_none(network)
        except ib_ex.InfobloxSearchError:
            ib_network = None
        return ib_network is not None

    def delete_network(self, network_view, cidr):
        network = InfobloxNetwork.load(network_view, cidr)
        self._delete_infoblox_object(network)
    ...
    def get_host_record(self, dns_view, ip):
        host_record = InfobloxHostRecord.load(dns_view, ip)
        hr_data = self._get_infoblox_object_or_none(host_record)

        if not hr_data:
            return None

        hr_class = host_record.parsing_class()
        return hr_class.from_dict(hr_data)
    ...
    def create_host_record_for_given_ip(self, dns_view, zone_auth,
        ...
        host_record = InfobloxHostRecord.load(dns_view, ip, name,
                                              mac, extattrs, use_dhcp)
        return self._create_infoblox_ip_address(host_record)
    ...
    def create_host_record_from_range(self, dns_view, network_view_name,
        ...
        name = '.'.join([hostname, zone_auth])
        ip = InfobloxIPAllocationObject.next_available_ip_from_range(
    ...
    def delete_host_record(self, dns_view, ip_address):
        host_record = InfobloxHostRecord.load(dns_view, ip_address)
        self._delete_infoblox_object(host_record)

    def create_fixed_address_for_given_ip(self, network_view, mac, ip,
                                          extattrs):
        fixed_address = InfobloxFixedAddress.load(network_view, ip,
                                                  mac, extattrs)
        return self._create_infoblox_ip_address(fixed_address)
    ...
    def create_fixed_address_from_cidr(self, netview, mac, cidr, extattrs):
        ip = InfobloxIPAllocationObject.next_available_ip_from_cidr(
            netview, cidr)

        fixed_address = InfobloxFixedAddress.load(netview, ip, mac, extattrs)
        return self._create_infoblox_ip_address(fixed_address)

    def delete_fixed_address(self, network_view, ip_address):
        fixed_address = InfobloxFixedAddress.load(network_view, ip_address)
        self._delete_infoblox_object(fixed_address)
    ...
    def bind_name_with_record_a(self, dns_view, ip, name, bind_list,
                                extattrs):
        records = InfobloxDNSRecord.load(dns_view, ip, name,
                                         bind_list, extattrs)
        for record in records:
            self._create_infoblox_object(record,
                                         check_if_exists=True,
                                         update_if_exists=False)

    def unbind_name_from_record_a(self, dns_view, ip, name, unbind_list):
        records = InfobloxDNSRecord.load(dns_view, ip, name, unbind_list,
                                         mode='delete')
        for record in records:
            self._delete_infoblox_object(record)
    ...
    def _create_infoblox_ip_address(self, ip_object):
        try:
            created_ip_json = self._create_infoblox_object(
                ip_object, check_if_exists=False)
            parsing_class = ip_object.parsing_class()
            return parsing_class.from_dict(created_ip_json)
        except ib_ex.InfobloxCannotCreateObject as e:
            if "Cannot find 1 available IP" in e.response['text']:
                raise ib_ex.InfobloxCannotAllocateIp(
                    ip_data=ip_object.payload)
            else:
                raise e
