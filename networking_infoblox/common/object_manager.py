...
class IPv4Backend(IPBackend):
    ...
    def create_host_record(use_dhcp):
        return objects.HostRecordIPv4(use_dhcp)
...
class IPv6Backend(IPBackend):
    ...
    @staticmethod
    def create_host_record(use_dhcp):
        return objects.HostRecordIPv6(use_dhcp)
...
class InfobloxFixedAddress(InfobloxObject):
    ...
    def load(cls, network_view, ip, mac=None, extattrs=None,
             use_dhcp=True):
        if InfobloxObject._determine_ip_version(ip) == 6:
            return InfobloxHostRecordV6(network_view, ip, mac,
                                        extattrs, use_dhcp)
        else:
            return InfobloxHostRecordV4(network_view, ip, mac,
                                        extattrs, use_dhcp)
...
class InfobloxFixedAddressV4(InfobloxIPRange):
...
class InfobloxFixedAddressV6(InfobloxIPRange):
    def __init__(self, network_view, ip, mac, extattrs,
                 use_dhcp):
        ...
        ips = self._prepare_ip(ip, mac, use_dhcp)
        self.payload.update({'ipv6addrs': ips,
                             'duid': ib_utils.generate_duid(mac)})
