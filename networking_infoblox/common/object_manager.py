...
class InfobloxNetwork(InfobloxObject):
    # Note(pbondar): Different signature for __new__ and __init__
    # is not a good thing to have (however it works),
    # it confuses static code analyzers and complicates code reading,
    # so looking for better solution.
    ...
    def __new__(cls, dnsview_name, ip, name=None, extattrs=None):
        if InfobloxObject._determine_ip_version(ip) == 6:
            return super(InfobloxHostRecord, cls).__new__(
                InfobloxHostRecordV6)
        else:
            return super(InfobloxHostRecord, cls).__new__(
                InfobloxHostRecordV4)
...
class InfobloxHostRecordV4(InfobloxHostRecord):
    def __init__(self, dnsview_name, ip, mac=None, name=None, extattrs=None,
                 use_dhcp=True):
        super(InfobloxHostRecordV4, self).__init__(
            'record:host', 4, dnsview_name, ip, name, extattrs)
        ipv4 = objects.IPv4(ip, mac, use_dhcp)
        self.payload.update({'ipv4addrs': ipv4.to_dict()})
        self.return_fields = ['ipv4addrs']

    def parsing_class(self):
        return objects.HostRecordIPv46
...
class InfobloxObjectManager(object):
    ...
    def delete_network(self, network_view, cidr):
        network = InfobloxNetwork(network_view, cidr)
    ...
    def get_host_record(self, dns_view, ip):
        host_record = InfobloxHostRecord(dns_view, ip)
    ...
    def delete_host_record(self, dns_view, ip_address):
        host_record = InfobloxHostRecord(dns_view, ip_address)
