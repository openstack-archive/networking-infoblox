...
class InfobloxPool(subnet_alloc.SubnetAllocator):
    ...
    def _get_changed_pools(nios_pools, pools_from_request, ip_version):
        """Calculates difference between current and existent ranges,
        ...
        add = new_pools - old_pools
        remove = old_pools - new_pools

        add_list = [request_pool_map[pool] for pool in add]
        remove_list = [nios_pool_map[pool] for pool in remove]
    ...
    def update_subnet(self, subnet_request):
        ...
        add, remove = self._get_changed_pools(
            nios_pools,
        ...
        self._allocate_pools(add, subnet_request.subnet_cidr)
