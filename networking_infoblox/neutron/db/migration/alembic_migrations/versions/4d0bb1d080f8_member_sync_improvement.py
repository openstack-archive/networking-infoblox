...
def update_gm_ids():
    ...
    sub_qry = session.query(infoblox_grids.c.grid_id)
    sub_qry = sub_qry.filter(infoblox_grids.c.grid_status == 'ON')
    q = session.query(infoblox_grid_members).filter(
        infoblox_grid_members.c.grid_id.in_(sub_qry))
    q = q.filter(infoblox_grid_members.c.member_type == 'GM')
    gm_row = q.first()
    if gm_row:
