...
def reset_ea_for_network(ib_network):
    ...
    # reset cloud EAs only, leaving custom EAs alone
    reset_keys = [const.EA_ACCOUNT, const.EA_TENANT_NAME,
                  const.EA_NETWORK_VIEW_ID,
                  const.EA_IS_EXTERNAL, const.EA_IS_SHARED, const.EA_SUBNET_ID,
                  const.EA_SUBNET_NAME, const.EA_NETWORK_ID,
                  const.EA_NETWORK_NAME, const.EA_NETWORK_ENCAP,
                  const.EA_SEGMENTATION_ID, const.EA_PHYSICAL_NETWORK_NAME]
    eas = ib_network.extattrs
    ea_dict = eas.to_dict()
    map(lambda ea: ea_dict.pop(ea, None), reset_keys)
    ib_network.extattrs = ib_objects.EA(ea_dict)
...
def reset_ea_for_range(ib_range):
    ...
    ea_dict = eas.to_dict()
    map(lambda ea: ea_dict.pop(ea, None), reset_keys)
    ib_range.extattrs = ib_objects.EA(ea_dict)
...
def reset_ea_for_zone(ib_zone):
    ...
    ea_dict = eas.to_dict()
    map(lambda ea: ea_dict.pop(ea, None), reset_keys)
    ib_zone.extattrs = ib_objects.EA(ea_dict)
