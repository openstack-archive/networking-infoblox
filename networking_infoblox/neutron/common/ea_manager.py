...
def get_dict_for_ip(user_id, tenant_id, port_id, device_owner, device_id,
                    cloud_api_owned, vm_id, ip_type):
    return {'Tenant ID': tenant_id,
            'Account': user_id,
            'Port ID': port_id,
            'Port Attached Device - Device Owner': device_owner,
            'Port Attached Device - Device ID': device_id,
            'Cloud API Owned': cloud_api_owned,
            'IP Type': ip_type,
            'VM ID': vm_id}


def get_default_ea_for_ip(user_id, tenant_id):
    ip_dict = get_dict_for_ip(user_id, tenant_id, None, None,
                              None, True, None, 'Fixed')
    return objects.EA(ip_dict)
...
def get_ea_for_ip(user_id, tenant_id, network, port_id, device_id,
    ...
    common_ea = get_common_ea(network)
    ip_dict = get_dict_for_ip(user_id, tenant_id, port_id, device_owner,
                              device_id, common_ea['Cloud API Owned'],
                              instance_id, 'Fixed')
