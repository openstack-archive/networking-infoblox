...
class GridMappingManager(object):
    ...
    def __init__(self, context, connector, grid_config):
        ...
        self.grid_id = self._grid_config.grid_id
    ...
    def sync(self):
        ...
        self.db_members = dbi.get_members(session, grid_id=self.grid_id)
        ...
        self._sync_network_views(discovered_network_views,
    ...
    def _sync_network_mapping(self, discovered_networks,
        ...
        addable_mapping_members = list(addable_set)
        for mapping_member_info in addable_mapping_members:
            mapping_member = mapping_member_info.split(':')
        ...
        removable_mapping_members = list(removable_set)
        for mapping_member_info in removable_mapping_members:
            mapping_member = mapping_member_info.split(':')
            network_view_id = mapping_member[0]
            member_id = mapping_member[1]
    ...
    def _get_mapping_members(self, discovered_networks,
        ...
        for netview in discovered_delegations:
            ...
            mapping_row_info = (netview_id + ':' + authority_member + ':' +
                                mapping_relation)
        ...
        for network in discovered_networks:
            ...
            mapping_member_info = (netview_id + ':' + authority_member + ':' +
                                   mapping_relation)
    ...
    def _update_mapping_conditions(self, discovered_netview, netview_id):
        ...
        for condition_name in mapping_conditions:
            conditions = [netview_id + ':' + condition_name + ':' + value
        ...
        addable_conditions = list(addable_set)
        for condition_attr in addable_conditions:
            condition = condition_attr.split(':')
        ...
        removable_conditions = list(removable_set)
        for condition_attr in removable_conditions:
            condition = condition_attr.split(':')
            network_view_id = condition[0]
            neutron_object_name = condition[1]
            neutron_object_value = condition[2]
