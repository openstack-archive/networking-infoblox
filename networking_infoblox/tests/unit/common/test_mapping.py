...
class GridMappingTestCase(base.TestCase, testlib_api.SqlTestCase):
    ...
    def setUp(self):
        ...
        self.member_mgr = member.GridMemberManager(self.ctx, self.connector,
        ...
        for netview in expected_conditions:
            ...
            for condition_name in expected_conditions[netview]:
                ...
                if not isinstance(values, list):
                    expected_condition_rows.append(netview_id + ':' +
                                                   condition_name + ':' +
                                                   values)
                    continue
                for value in values:
    ...
    def test_sync_for_cloud(self):
        ...
        mapping_mgr = mapping.GridMappingManager(self.ctx, self.connector,
                                                 self.test_grid_config)
