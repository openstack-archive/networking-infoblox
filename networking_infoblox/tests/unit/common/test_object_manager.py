...
class ObjectManipulatorTestCase(base.BaseTestCase):
    ...
    def test_bind_names_with_a_record(self):
        ...
        exp_for_ptr = {'view': dns_view_name,
                       'ipv4addr': ip,
                       'ptrdname': name}
        calls = [mock.call('record:a', exp_for_a, None, proxy=mock.ANY),
                 mock.call('record:ptr', exp_for_ptr, None, proxy=mock.ANY)]
        connector.get_object.assert_has_calls(calls)

        create_calls = [mock.call('record:a', exp_for_a, None),
                        mock.call('record:ptr', exp_for_ptr, None)]
    ...
    def test_delete_fixed_address(self):
        ...
        payload = {'network_view': network_view,
                   'ipv4addr': ip}
        connector.get_object.assert_called_once_with(
            'fixedaddress', payload, None, proxy=mock.ANY)
        connector.delete_object.assert_called_once_with(mock.ANY)
...
class TestInfobloxNetwork(base.BaseTestCase):
    ...
    def test_v4_network_is_loaded(self):
        network = om.InfobloxNetwork.load('some-view', '192.168.1.0/24')
    ...
    def test_v6_network_is_loaded(self):
        network = om.InfobloxNetwork.load('some-view', 'ffde:2332::/24')
...
class TestInfobloxHostRecord(base.BaseTestCase):
    ...
    def test_v4_hostrecord_is_loaded(self):
        host = om.InfobloxHostRecord.load('some-dns-view', '192.168.1.25')
        ...
        self.assertEqual('192.168.1.25',
                         host.payload['ipv4addrs'][0]['ipv4addr'])

    def test_v6_network_is_loaded(self):
        host = om.InfobloxHostRecord.load('some-dns-view', 'ffde:2332::12')
        self.assertIsInstance(host, om.InfobloxHostRecordV6)
        self.assertEqual(6, host.ip_version)
        self.assertEqual('record:host', host.object_type)
        self.assertEqual('ffde:2332::12',
                         host.payload['ipv6addrs'][0]['ipv6addr'])


class TestInfobloxIPRange(base.BaseTestCase):

    def test_v4_hostrecord_is_loaded(self):
        host = om.InfobloxIPRange.load('some-view', '192.168.1.25',
                                       '192.168.1.40')
    ...
    def test_v6_network_is_loaded(self):
        host = om.InfobloxIPRange.load('some-view', 'ffde::12', 'ffde::ffce')
