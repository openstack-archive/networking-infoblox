...
class IpamAsyncControllerTestCase(base.TestCase, testlib_api.SqlTestCase):
    ...
    def test_create_network_sync_without_dhcp_relay(self):
        test_opts = dict()
        self.helper.prepare_test(test_opts)
        ...
        assert not self.ib_cxt.ibom.create_fixed_address_from_cidr.called

    @mock.patch.object(utils, 'get_ip_version', mock.Mock())
    def test_create_network_sync_with_dhcp_relay(self):
        test_opts = dict()
        self.helper.prepare_test(test_opts)
        self.ib_cxt.grid_config.dhcp_relay_management_network = '11.11.1.0/24'
        self.ib_cxt.grid_config.dhcp_relay_management_network_view = 'default'

        ipam_controller = ipam.IpamAsyncController(self.ib_cxt)
        ipam_controller.create_network_sync()

        assert self.ib_cxt.ibom.create_fixed_address_from_cidr.called
    ...
    @mock.patch.object(dbi, 'get_management_ip')
    @mock.patch.object(dbi, 'get_network_view_mappings', return_value=[])
    def test_delete_network_sync_without_dhcp_relay_and_mapping(
            self, netview_mapping_mock, mgmt_ip_mock):
        test_opts = dict()
        self.helper.prepare_test(test_opts)
        network_id = 'test-id'
        ...
        assert not mgmt_ip_mock.called
        assert not self.ib_cxt.ibom.delete_object_by_ref.called
        assert not self.ib_cxt.ibom.has_networks.called
        assert not self.ib_cxt.ibom.delete_network_view.called

    @mock.patch.object(dbi, 'get_management_ip')
    @mock.patch.object(dbi, 'get_network_view_mappings', return_value=[])
    def test_delete_network_sync_with_dhcp_relay_and_without_mapping(
            self, netview_mapping_mock, mgmt_ip_mock):
        test_opts = dict()
        self.helper.prepare_test(test_opts)
        self.ib_cxt.grid_config.dhcp_relay_management_network = '11.11.1.0/24'
        ...
        assert mgmt_ip_mock.called
        assert self.ib_cxt.ibom.delete_object_by_ref.called
        assert not self.ib_cxt.ibom.has_networks.called
        assert not self.ib_cxt.ibom.delete_network_view.called
    ...
    @mock.patch.object(dbi, 'get_management_ip')
    @mock.patch.object(dbi, 'get_network_view_mappings')
    def test_delete_network_sync_with_dhcp_relay_and_mapping_and_shared(
            self, netview_mapping_mock, mgmt_ip_mock, find_row_mock):
        test_opts = dict()
        self.helper.prepare_test(test_opts)
        self.ib_cxt.grid_config.dhcp_relay_management_network = '11.11.1.0/24'
        self.ib_cxt.grid_config.dhcp_relay_management_network_view = 'default'
    ...
    @mock.patch.object(dbi, 'get_management_ip')
    @mock.patch.object(dbi, 'get_network_view_mappings')
    def test_delete_network_sync_with_relay_n_mapping_n_not_shared_n_nework(
            self, netview_mapping_mock, mgmt_ip_mock, find_row_mock):
        test_opts = dict()
        self.helper.prepare_test(test_opts)
        self.ib_cxt.grid_config.dhcp_relay_management_network = '11.11.1.0/24'
    ...
    @mock.patch.object(dbi, 'get_management_ip')
    @mock.patch.object(dbi, 'get_network_view_mappings')
    def test_delete_network_sync_with_relay_n_mapping_n_not_shared_n_no_nework(
            self, netview_mapping_mock, mgmt_ip_mock, find_row_mock):
        test_opts = dict()
        self.helper.prepare_test(test_opts)
        self.ib_cxt.grid_config.dhcp_relay_management_network = '11.11.1.0/24'
