...
class TestInfobloxConnector(base.TestCase):
    def setUp(self):
        ...
        opts = self._prepare_options(self)

        self.connector = connector.HttpClient(opts)

    def _prepare_options(self):
        return dict(wapi_url='https://infoblox.example.org/wapi/v1.1/',
                    username='admin',
                    password='password',
                    ssl_verify=True,
                    wapi_http_pool_connections=10,
                    wapi_http_pool_maxsize=10,
                    wapi_http_request_timeout=10)
