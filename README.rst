...
database with data about the Infoblox grid. This enables the the selection
...
not directly part of the IPAM function. For example, it will listen for L2
network creation and create zones based upon those networks. Neutron does
not call the IPAM function when managing networks (only subnets), so the agent
