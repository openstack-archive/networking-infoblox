...
protocol in that deployment model. Note that there is a known issue that
the GM is always in the selection list, even if it is not configured and
licensed for DNS or DHCP.
